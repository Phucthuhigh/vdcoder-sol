...
const int N = 20, mod[] = { (int)1e9 + 2277, (int)1e9 + 5277, (int)1e9 + 8277, (int)1e9 + 9277 };
int n, a[N], res = N;

void Try(int i, ll s1, ll s2, int skip)
{
    if (i > n)
    {
        if (s1 == s2) res = min(res, skip);
        return;
    }

    Try(i + 1, s1 + a[i], s2, skip);
    Try(i + 1, s1, s2 + a[i], skip);
    Try(i + 1, s1, s2, skip + 1);
}
